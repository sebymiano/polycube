...
TransparentCube::TransparentCube(const std::string &name,
    ...
    : BaseCube(name, service_name, "", patch_panel_ingress_,
     ...
     ingress_next_(0), egress_next_(0), attach_(attach),
     parent_(nullptr) {
...
std::string TransparentCube::get_parent_parameter(const std::string &parameter) {
  if (!parent_) {
...
void TransparentCube::set_parameter(const std::string &parameter, const std::string &value) {
...
void TransparentCube::send_packet_out(const std::vector<uint8_t> &packet,
  ...
  Controller &c = (get_type() == CubeType::TC) ?
    Controller::get_tc_instance() : Controller::get_xdp_instance();
