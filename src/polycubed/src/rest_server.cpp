...
void RestServer::attach(const Rest::Request &request,
  ...
  if (val.find("after") != val.end()) {
    if (position_) {
      response.send(Http::Code::Bad_Request, "position and after cannot be used together");
      return;
    }
  ...
  if (val.find("before") != val.end()) {
    if (position_) {
      response.send(Http::Code::Bad_Request, "position and before cannot be used together");
      return;
    }

    if (!other.empty()) {
      response.send(Http::Code::Bad_Request, "after and before cannot be used together");
      return;
    }
