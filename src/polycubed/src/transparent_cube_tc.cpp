...
std::string TransparentCubeTC::get_wrapper_code() {
  return TransparentCube::get_wrapper_code() + CubeTC::CUBE_TC_COMMON_WRAPPER + TRANSPARENTCUBETC_WRAPPER;
}
...
void TransparentCubeTC::do_compile(int id, uint32_t next, ProgramType type,
  ...
  cflags_.push_back(std::string("-DTYPE=" + std::to_string(static_cast<int>(type))));
