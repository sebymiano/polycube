...
void PolycubedCore::attach(const std::string &cube_name,
  ...
  if (std::regex_match(port_name, match, rule)) {
    ...
    if (!cube2) {
      throw std::runtime_error("Cube " + std::string(match[1]) + " is transparent");
    }
    ...
    case PortType::TC:
      if (cube->get_type() != CubeType::TC) {
        throw std::runtime_error(cube_name + " and " + port_name + " have incompatible types");
      }
      break;
    case PortType::XDP:
      if (cube->get_type() != CubeType::XDP_DRV && cube->get_type() != CubeType::XDP_DRV) {
        throw std::runtime_error(cube_name + " and " + port_name + " have incompatible types");
      }
      break;
    }
    ...
    if (ServiceController::ports_to_ifaces.count(port_name) == 0) {
      ...
      ServiceController::ports_to_ifaces.emplace(std::piecewise_construct,
                              std::forward_as_tuple(port_name),
                              std::forward_as_tuple(std::move(iface)));
    }

    //peer = dynamic_cast<PeerIface *>(ServiceController::ports_to_ifaces.at(port_name).get());
...
std::string PolycubedCore::get_cube_port_parameter(
  const std::string &cube, const std::string &port, const std::string &parameter) {
  ...
  std::string url("/" + service_name + "/" + cube + "/ports/" + port + "/" + parameter);
  auto req = HttpHandleRequest(polycube::service::Http::Method::Get,
    url, "");
...
std::string PolycubedCore::set_cube_parameter(const std::string &cube,
  ...
  auto req = HttpHandleRequest(polycube::service::Http::Method::Put,
    url, "");
  auto res = HttpHandleResponse();
