...
TransparentCubeXDP::TransparentCubeXDP(const std::string &name,
...
std::string TransparentCubeXDP::get_wrapper_code() {
  return TransparentCube::get_wrapper_code() + \
    CubeXDP::CUBEXDP_COMMON_WRAPPER + \
...
void TransparentCubeXDP::compileEgress(ebpf::BPF &bpf, const std::string &code,
                                       uint16_t next) {
  TransparentCubeTC::do_compile(get_id(), next, ProgramType::EGRESS, level_, bpf, code, 0);
}
