...
std::shared_ptr<CubeIface> CubeFactoryImpl::create_cube(const std::string &name,
  ...
  typename std::unordered_map<std::string, std::shared_ptr<BaseCubeIface>>::iterator iter;
  bool inserted;
...
std::shared_ptr<TransparentCubeIface> CubeFactoryImpl::create_transparent_cube(
  ...
  case CubeType::XDP_SKB:
  case CubeType::XDP_DRV:
    cube = std::make_shared<TransparentCubeXDP>(name, service_name_, ingress_code,
                                     egress_code, level, type, attach);
    break;
  case CubeType::TC:
    cube = std::make_shared<TransparentCubeTC>(name, service_name_, ingress_code,
                                    egress_code, level, attach);
    break;
  default:
    throw std::runtime_error("invalid cube type");
  }
