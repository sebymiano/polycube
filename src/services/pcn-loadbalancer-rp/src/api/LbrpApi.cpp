...
using namespace polycube::service::api::LbrpApiImpl;
...
Response read_lbrp_dyn_opt_by_id_handler(
  const char *name, const Key *keys,
  size_t num_keys ) {
  // Getting the path params
  std::string unique_name { name };
  ...
  try {
    ...
    auto x = read_lbrp_dyn_opt_by_id(unique_name);
    nlohmann::json response_body;
    response_body = x;
    return { kOk, ::strdup(response_body.dump().c_str()) };
  } catch(const std::exception &e) {
...
Response read_lbrp_service_name_by_id_handler(
  ...
  try {
    ...
    auto x = read_lbrp_service_name_by_id_0(unique_name, unique_vip, unique_vport, unique_proto_);
    nlohmann::json response_body;
    response_body = x;
    return { kOk, ::strdup(response_body.dump().c_str()) };
  } catch(const std::exception &e) {
...
Response read_lbrp_start_morpheus_by_id_handler(
  const char *name, const Key *keys,
  size_t num_keys ) {
  // Getting the path params
  std::string unique_name { name };
  ...
  try {
    ...
    auto x = read_lbrp_start_morpheus_by_id(unique_name);
    nlohmann::json response_body;
    response_body = x;
    return { kOk, ::strdup(response_body.dump().c_str()) };
  } catch(const std::exception &e) {
...
Response update_lbrp_start_morpheus_by_id_handler(
  const char *name, const Key *keys,
  size_t num_keys ,
  const char *value) {
  // Getting the path params
  ...
  try {
    ...
    update_lbrp_start_morpheus_by_id(unique_name, unique_value);
    return { kOk, nullptr };
  } catch(const std::exception &e) {
    return { kGenericError, ::strdup(e.what()) };
  }
